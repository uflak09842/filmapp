...
const WatchedMovies = () => {
    ...
    const getMovies = async (page = 1, shouldAppend = false) => {
        try {
            ...
            const response = await axiosInstance.get(
                `${process.env.EXPO_PUBLIC_SERVER_URL}/userWatched?page=${page}&limit=12`
            );
            ...
            console.error('İzlenen filmler alınırken hata:', err);
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    const handleWatchRemove = async (id) => {
        try {
            setLoading(true);
            const response = await axiosInstance.delete(process.env.EXPO_PUBLIC_SERVER_URL + '/deleteWatch', {
                data: {
                    mvId: id
                }
            });
            ...
            console.error('İzlenen film silinirken hata:', err);
            throw err;
        } finally {
            setLoading(false);
        }
